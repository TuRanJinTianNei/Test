...
clear all
close all
SNR=-10:1:0;             %信噪比取值，以db为单位
kkk=100;

fprintf('========================================\n');
fprintf('信号存在性检测 - 蒙特卡洛仿真开始\n');
fprintf('SNR范围: %.0f dB 到 %.0f dB\n', min(SNR), max(SNR));
fprintf('每个SNR仿真次数: %d\n', kkk);
fprintf('总仿真次数: %d\n', length(SNR)*kkk);
fprintf('========================================\n\n');

for index=1:length(SNR)
    fprintf('正在处理 SNR = %.0f dB (%d/%d)...\n', SNR(index), index, length(SNR));
    tic;  % 开始计时
    for p=1:kkk
        % 每10次显示一次进度
        if mod(p, 10) == 0 || p == 1
            fprintf('  进度: %d/%d (%.1f%%)\r', p, kkk, p/kkk*100);
        end
    %% 信号1
    ...
    fprintf('  完成! 准确率: %.2f%% (%d/%d) 耗时: %.2f秒\n\n', ...
        corr(index)*100, sum(indx), kkk, elapsed_time);
end
