...
for index=1:length(SNR)
    ...
    if SHOW_FIGURES && p == 1
        ...
        sgtitle('【第一阶段】两信号叠加检测 - 算法流程可视化','FontSize',16,'FontWeight','bold','Color',[0.2 0.2 0.2]);
        subplot(2,3,1);
    ...
    fprintf('完成 | 正确率: %5.2f%% (%d/%d) | 耗时: %.2f秒\n', ...
        corr1(index)*100, sum(indx), n, elapsed_time);
end
fprintf('----------------------------------------\n');
fprintf('第一阶段完成！\n\n');
...
for index=1:length(SNR)
    ...
    if SHOW_FIGURES && p == 1
        figure(2);
        set(gcf,'Position',[50,50,1400,900],'Color','white');
        sgtitle('【第二阶段】单信号检测（非线性信道） - 算法流程可视化','FontSize',16,'FontWeight','bold','Color',[0.2 0.2 0.2]);
        subplot(2,3,1);
        plot(f,Pxx,'-','LineWidth',2,'Color',[0.2 0.4 0.8]);
        xlabel('归一化频率','FontSize',11,'FontWeight','bold');
        ylabel('功率谱密度 (dB)','FontSize',11,'FontWeight','bold');
    ...
    elapsed_time = toc;
    fprintf('完成 | 正确率: %5.2f%% (%d/%d) | 耗时: %.2f秒\n', ...
        corr2(index)*100, sum(indx), n, elapsed_time);
end
fprintf('----------------------------------------\n');
fprintf('第二阶段完成！\n\n');

fprintf('========================================\n');
fprintf('所有仿真完成！正在绘制结果曲线...\n');
fprintf('========================================\n');
