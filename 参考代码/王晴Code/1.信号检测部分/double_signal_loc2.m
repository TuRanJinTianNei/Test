...
f_=f(floor(length(Pxx_int)*0.227):floor(length(Pxx_int)*LocN),1);
Pxx_=Pxx(floor(length(Pxx_int)*0.227):floor(length(Pxx_int)*LocN),1);
plot(f_,Pxx_)
title("Welch法功率谱估计")
for i=2:length(Pxx_)-1
    n1(i)=(loc(i,2)-(loc(i-1,2)));
    n2(i)=(loc(i+1,2)-(loc(i,2)));
    d(i)=n2(i)-n1(i);
end
p=1:length(d);
figure(5)
