...
if use_channel
    ...
    % 固定文件名
    mat_filename_rx = 'RX_signal.mat';
    
    % 保存接收信号和相关参数
    save(mat_filename_rx, 'Rx_data', 'Tx_data', 'fs', 'carrier_count', ...
        'subcarrier_spacing', 'IFFT_bin_length', 'GI', 'GI_first_symbol', 'GIP', ...
        'total_symbols', 'symbols_per_frame', 'targetSNRdB', 'use_rayleigh_fading', ...
        'fd', 'Tx_signal_power', 'noise_sigma', '-v7.3');
    
    fprintf('接收信号已保存到文件: %s\n', mat_filename_rx);
    fprintf('  包含变量: Rx_data, Tx_data, fs, carrier_count, ...\n');
...
pathDelays = [0, 0.1e-6, 0.3e-6, 0.6e-6, 0.9e-6, 1.5e-6];  % Path delays (s) - 六径：0秒（主径），0.1, 0.3, 0.6, 1.2, 2.5微秒（多径）
