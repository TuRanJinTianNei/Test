%实现基本的OFDM通信系统仿真，包括QPSK调制、串并变换、IFFT、并串变换、
%每个符号采用1024个子载波，每个载波传输6个符号，添加的保护间隔为256个。
clc
clear all
close all
%参数设置
SubCarryN=128;                   %子载波数
SubCarry_eff=SubCarryN*1/4;      %有效子载波数
ratio=1/4;                %CP length ratio
fftLen=SubCarryN;         %FFT length
SymbN=100000;             %number of OFDM symbols
GuardLen=SubCarryN*ratio;    %CP length
SymLen=SubCarryN+GuardLen;  %length of each symbol after cp
beta=1/32; %rolloff ratio
SNR_dB=5;             %信噪比取值，以db为单位
% SNR_dB=5;%设置信噪比db
SNR=10*log10(SNR_dB);
%%----------------------------------------发射端-----------------------------------------------------%%
%输入比特序列长度=子载波数×符号数×每符号比特数
% for index=1:length(SNR)
% for p=1:200
SignalLen=SubCarry_eff*SymbN;
M=4;                                  %调制阶数
Signal=randi([0 M-1],1,SignalLen);
if M==4
    Signal=pskmod(Signal,M,pi/4);%
else
    Signal=qammod(Signal,M);
end
ParaSymSig=reshape(Signal,SubCarry_eff,SymbN);
% plot(ParaSymSig,'*r');
%在中间补零
% x=[ParaSymSig(1:SubCarryN*3/8,1:SymbN);zeros(SubCarryN/4,SymbN);ParaSymSig(SubCarryN*3/8+1:SubCarry_eff,1:SymbN)];%有效子载波占3/4
x=[ParaSymSig(1:SubCarryN*1/8,1:SymbN);zeros(SubCarryN*3/4,SymbN);ParaSymSig(SubCarryN*1/8+1:SubCarry_eff,1:SymbN)];%有效子载波占1/4
y=ifft(x,fftLen,1);   %通过傅里叶反变换，将频域数据转化为时域数据，1024行，500列

%在数组中加循环前缀的方法
y_CP=[y(fftLen-GuardLen+1:fftLen,:);y]; %加入循环前缀

%rcosfilter window 
window_coff=rcoswindow(beta,SymLen);
y_window=zeros(SymLen,SymbN);
for n=1:SymbN
    y_window(:,n)=y_CP(:,n).*window_coff.';
end
% y_window=y_CP;

%并串变换
TrData=reshape(y_window,1,SymLen*SymbN);%并串变换，转换成一串，变成一行，7680列

%%Following is used to check the spectrum of transmitted signals
% TrData1=awgn(TrData,snr,'measured');
% SymbN_per=5; %number of symbols in per segment
% perLen=SymLen*SymbN_per; %length of per segment
% perFFT(1:perLen)=0;
% ave_partTrDataFFT=0;
% for n=0:9 %count with 10 segments
%     partTrData=TrData1(n*perLen+1:(n+1)*perLen); %into segment
%     partTrDataFFT=abs(fft(partTrData));
%     ave_partTrDataFFT=ave_partTrDataFFT+partTrDataFFT;
% end
% ave_partTrDataFFT=ave_partTrDataFFT/10;
% TrDataFFT_log=20*log10(ave_partTrDataFFT);
% figure;
% plot((0:perLen-1)/perLen,TrDataFFT_log);
%
%瑞丽信道参数设置
%%Rayleigh channel generated by using randn
% P_hdB=[0,-17,-18,-30];%各信道的功率特征
P_hdB=[0,0,0,0];%各信道的功率特征
D_h=[0,30,60,200];%各信道的延迟（采样点）
P_h=10.^(P_hdB/10);%各信道的功率特性
NH=length(P_hdB);%多径信道的个数
LH=D_h(end)+1;%信道长度（延迟过后）

% A_h=(randn(1,NH)+sqrt(-1)*randn(1,NH)).*sqrt(P_h/2);%瑞丽随机变量
% h=zeros(1,LH);%初始化信道冲击响应模型
% h(D_h+1)=A_h;%信道冲击响应（同时体现出衰减系数和信道时延）
% rx_channel=conv(TrData,h,'full');%卷积，输出通过该信道的信号
% ReData=awgn(rx_channel,SNR,'measured','dB');
% 
% r1=ReData;           %接收数据（可以选取一部分）
% r=reshape([r1(1:SymLen*SymbN)],SymLen,SymbN);
% %去循环前缀
% rev_signal=r(GuardLen+1:SymLen,:);%去掉循环前缀并恢复格式
% rece_signal=fft(rev_signal);
for N=1:length(SNR_dB)
%% The channel varies among different symbols
rece_signal=zeros(SubCarryN,SymbN);
A_h=(randn(1,NH)+sqrt(-1)*randn(1,NH)).*sqrt(P_h/2);%瑞丽随机变量
Eh21=0;
Eh42=0;
Eh63=0;
for n=1:SymbN   
    h=zeros(1,LH);%初始化信道冲击响应模型
    h(D_h+1)=A_h;%信道冲击响应（同时体现出衰减系数和信道时延）

    rx_channel=conv(y_window(:,n),h.','full');%卷积，输出通过该信道的信号
    ReData=awgn(rx_channel,SNR_dB(N),'measured');%瑞丽信道
%     ReData=awgn(TrData,SNR(N),'measured');%awgn信道
    r1=ReData;           %接收数据（可以选取一部分）
%     去循环前缀
    rev_signal=r1(GuardLen+1:SymLen,1);%去掉循环前缀并恢复格式
    rece_signal(:,n)=fft(rev_signal);
    
    FFTh=fft(A_h,fftLen);
%     Eh21=Eh21+abs(FFTh).^2;
%     Eh42=Eh42+abs(FFTh).^4;
%     Eh63=Eh63+abs(FFTh).^6;
    A_h=(randn(1,NH)+sqrt(-1)*randn(1,NH)).*sqrt(P_h/2);%channel varies    
end
% Eh21=Eh21/SymbN;
% Eh42=Eh42/SymbN;
% Eh63=Eh63/SymbN;
% a1=Eh42./Eh21.^2; %To check E(|h|^2)
% a2=Eh63./Eh21.^3; %To check E(|h|^4)
%尝试多个符号累加
%rece_signal=reshape(rece_signal,1024,SymbN);
F1=zeros(1,SubCarryN);%F1是识别空载波的特征量
% F1=zeros(1,SubCarryN);%F1是识别空载波的特征量
%识别空载波
F1countLen=SymbN;
for i=1:SubCarryN
    [cumulants11]=func_get_cumulants(rece_signal(i,:));
    F1(i)=cumulants11(7);%=C21, the power of each subcarrier
%     F1(i)=cumulants11(8);%=C42/((C21)^2)
end
%画出F1的分布
figure;
i=1:SubCarryN;
plot(i,F1);hold on;

%%

% % 通过识别出的空载波求N
% %矩阵，用来存F1的数值
F1_B=0.1*max(F1);%门限
[F1v,F1index]=find(F1<F1_B);
EmptySub_Num=length(F1index);  %Number of empty subcarries
NoEmptySub_Num=SubCarryN-EmptySub_Num; %Number of non-empty subcarries

EmptySub=rece_signal(F1index,:); %store received empty subcarries
NoEmptySub=rece_signal(setdiff([1:SubCarryN],F1index),:);%store received non-empty subcarries

NoisePower=mean(mean(abs(EmptySub.^2)));%power of noise
Sum_Power=mean(mean(abs(rece_signal.^2)));%power of sum
Sub_Power=mean(mean(abs(NoEmptySub.^2)));%power of sub

snr1=(Sum_Power-NoisePower)/NoisePower;
SNR_est=10*log10(snr1)
end




