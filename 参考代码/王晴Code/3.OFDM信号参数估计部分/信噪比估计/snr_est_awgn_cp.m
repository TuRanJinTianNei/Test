...
%参数设置
SubCarryN=128;                   %子载波数
SubCarry_eff=SubCarryN*1/4;      %有效子载波数32
ratio=1/4;                %CP length ratio，32
...
SubCarry_eff_cp=SubCarry_eff+GuardLen;
...
%     rece_signal=fft(r1,128);
      rece_signal=fft(rev_signal);
      [row,len]=size(rece_signal);
      %Fs=50;x=reshape(r1,1,row*len);y=abs(fft(x));f = (0:N-1)*Fs/N;plot(f,y)
% end
%尝试多个符号累加
%rece_signal=reshape(rece_signal,1024,SymbN);
F1=zeros(1,row);%F1是识别空载波的特征量
...
% %矩阵，用来存F1的数值
F1_B=0.5*max(F1);%门限
[F1v,F1index]=find(F1<F1_B);%F1index是位置
EmptySub_Num=length(F1index);  %Number of empty subcarries
NoEmptySub_Num=row-EmptySub_Num; %Number of non-empty subcarries

EmptySub=rece_signal(F1index,:); %store received empty subcarries
NoEmptySub=rece_signal(setdiff([1:row],F1index),:);%store received non-empty subcarries
...
noise = noise_part * (row/(row - NoEmptySub_Num));
signal = signal_part - noise_part * (NoEmptySub_Num/(row - NoEmptySub_Num));

snr=signal/noise;
SNR_est=10*log10(snr);
MSE(N)=abs(SNR_est-SNR_dB(N)).^2;
end
semilogy(SNR_dB,MSE)
