...
%     dataIn = randi([0 1],1,NS*64);            % 生成随机符号数据
    %% 调制
...
%     dataIn = randi([0 1],NS*64,1);            % 生成随机符号数据
%     dataMod_64APSK = apskmod(dataIn,APSK_64,acsii_64,'InputType','bit','OutputDataType','single');
    %ofdm
    ...
    %% 载波发送
    carrier_QPSK = exp(1j*2*pi*(fc+fm)/fs.*(1:length(signalSend_QPSK)));
    signalMod_QPSK = signalSend_QPSK.*carrier_QPSK';
    
    carrier_16QAM = exp(1j*2*pi*(fc+fm)/fs.*(1:length(signalSend_16QAM)));
    signalMod_16QAM = signalSend_QPSK.*carrier_16QAM';
    
    carrier_32QAM = exp(1j*2*pi*(fc+fm)/fs.*(1:length(signalSend_32QAM)));
    signalMod_32QAM = signalSend_32QAM.*carrier_32QAM';
    
    carrier_64QAM = exp(1j*2*pi*(fc+fm)/fs.*(1:length(signalSend_64QAM)));
    signalMod_64QAM = signalSend_64QAM.*carrier_64QAM';
    
    carrier_16APSK = exp(1j*2*pi*(fc+fm)/fs.*(1:length(signalSend_16APSK)));
    signalMod_16APSK = signalSend_16APSK.*carrier_16APSK';
    
    carrier_32APSK = exp(1j*2*pi*(fc+fm)/fs.*(1:length(signalSend_32APSK)));
    signalMod_32APSK = signalSend_32APSK.*carrier_32APSK';
    
    carrier_64APSK = exp(1j*2*pi*(fc+fm)/fs.*(1:length(signalSend_64APSK)));
    signalMod_64APSK = signalSend_64APSK.*carrier_64APSK';
    
    carrier_OFDM = exp(1j*2*pi*(fc+fm)/fs.*(1:length(signalMod_OFDM)));
    signalMod_OFDM = signalMod_OFDM.*carrier_OFDM';
    
%     rchannel = clone(rayleighChan);
    
    signalChannel_QPSK = rayleighChan(signalMod_QPSK);
...
%     signalAWGN_QPSK=rayleighChan(signalAWGN_QPSK);
    ...
    [signalChannel_16QAM,pathr] = rayleighChan(signalMod_16QAM);
...
%     signalAWGN_16QAM=rayleighChan(signalAWGN_16QAM);
    ...
    [signalChannel_32QAM,pathr] = rayleighChan(signalMod_32QAM);
...
%     signalAWGN_16QAM=rayleighChan(signalAWGN_16QAM);
    ...
    [signalChannel_64QAM,pathr]= rayleighChan(signalMod_64QAM);
...
%     signalAWGN_64QAM=rayleighChan(signalAWGN_64QAM);
    ...
    [signalChannel_16APSK,pathr]= rayleighChan(signalMod_16APSK);
...
%     signalAWGN_16APSK=rayleighChan(signalAWGN_16APSK);
    ...
    [signalChannel_32APSK,pathr]= rayleighChan(signalMod_32APSK);
...
%     signalAWGN_32APSK=rayleighChan(signalAWGN_32APSK);
    ...
    PAPR_32APSK(tt) = func_get_PAPR(signalRecv_32APSK');
        ...
        [signalChannel_64APSK,pathr]= rayleighChan(signalMod_64APSK);
...
%     signalAWGN_32APSK=rayleighChan(signalAWGN_32APSK);
    ...
    [signalChannel_OFDM,pathr_OFDM]= rayleighChan(signalMod_OFDM);
    signalAWGN_OFDM = awgn(signalChannel_OFDM,SNR(index),'measured');
%     signalAWGN_OFDM=rayleighChan(rayleighChan(signalAWGN_OFDM));
