...
meantimes=20;     %100次取平均，蒙特卡洛仿真次数
para1=[128,256,512];
for i=1:length(snr)
     SNR=10^(snr(i)/10);         %信号功率除以噪声功率
     sigma=1/sqrt(2*SNR);        %将信号功率归一化之后得到的噪声的功率，可以计算出噪声的幅度，即
    for j=1:meantimes
        ...
        c42_ofdm128(i,j)=rt_C42(rt_ofdm128);    %c42 四阶累积量
        ...
        c42_ofdm256(i,j)=rt_C42(rt_ofdm256);    %c42 四阶累积量
        ...
        c42_ofdm512(i,j)=rt_C42(rt_ofdm512);    %c42 四阶累积量
    end
end
...
if K==1
    ...
    hold on
    legend('c42_ofdm128','c42_ofdm256','c42_ofdm512','理论值');
    axis([snr(1) snr(end) -0.10 0.10]);
    title('不同子载波数量对C42值的影响')
