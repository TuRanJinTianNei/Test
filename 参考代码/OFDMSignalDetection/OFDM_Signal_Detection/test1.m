...
% 绘制发送信号和接收信号的时域图和频域图
%**************************************************************************
fprintf('正在生成发送和接收信号的时域频域图...\n');
plot_signal_start = tic;

% 生成发送信号（上变频后取实部）
sig_tx = sig_ofdm.*exp(1j*2*pi*fc/fs*(0:length(sig_ofdm)-1));
sig_tx_real = real(sig_tx);

% 生成接收信号（通过Rayleigh信道和AWGN）
sig_rx = sig_tx;
sig_rx = real(MUL_RAYLEIGH(sig_rx,itau,power,itn,length(itau),length(sig_rx),1/fs,fmax,0));
sig_rx = awgn(sig_rx,snr_plot,'measured');

% 选择显示的数据点数（避免图形过于密集）
max_points = 10000;  % 最多显示10000个点
if length(sig_tx_real) > max_points
    idx = round(linspace(1, length(sig_tx_real), max_points));
    sig_tx_display = sig_tx_real(idx);
    sig_rx_display = sig_rx(idx);
    t_display = idx / fs * 1e6;  % 转换为微秒
else
    sig_tx_display = sig_tx_real;
    sig_rx_display = sig_rx;
    t_display = (0:length(sig_tx_real)-1) / fs * 1e6;  % 转换为微秒
end

% 计算频域
N_fft = 2^nextpow2(length(sig_tx_real));  % FFT点数
freq = (0:N_fft/2-1) * fs / N_fft / 1e6;  % 转换为MHz

% 发送信号频域
sig_tx_fft = fft(sig_tx_real, N_fft);
sig_tx_spectrum = 20*log10(abs(sig_tx_fft(1:N_fft/2)));
sig_tx_spectrum = sig_tx_spectrum - max(sig_tx_spectrum);  % 归一化到dB

% 接收信号频域
sig_rx_fft = fft(sig_rx, N_fft);
sig_rx_spectrum = 20*log10(abs(sig_rx_fft(1:N_fft/2)));
sig_rx_spectrum = sig_rx_spectrum - max(sig_rx_spectrum);  % 归一化到dB

% 绘制时域图
figure('Name', '发送和接收信号时域对比')
subplot(2,1,1)
plot(t_display, sig_tx_display, 'b-', 'LineWidth', 1.5);
xlabel('时间 (μs)');
ylabel('幅度');
title('发送信号时域波形');
grid on;
legend('发送信号', 'Location', 'best');
xlim([t_display(1), t_display(end)]);

subplot(2,1,2)
plot(t_display, sig_rx_display, 'r-', 'LineWidth', 1.5);
xlabel('时间 (μs)');
ylabel('幅度');
title('接收信号时域波形 (Rayleigh信道 + AWGN)');
grid on;
legend('接收信号', 'Location', 'best');
xlim([t_display(1), t_display(end)]);

% 绘制频域图
figure('Name', '发送和接收信号频域对比')
subplot(2,1,1)
plot(freq, sig_tx_spectrum, 'b-', 'LineWidth', 1.5);
xlabel('频率 (MHz)');
ylabel('功率谱密度 (dB)');
title('发送信号频域');
grid on;
...
subplot(2,1,2)
plot(freq, sig_rx_spectrum, 'r-', 'LineWidth', 1.5);
xlabel('频率 (MHz)');
ylabel('功率谱密度 (dB)');
title('接收信号频域 (Rayleigh信道 + AWGN)');
grid on;
legend('接收信号', 'Location', 'best');
xlim([0, fs/2/1e6]);

plot_signal_time = toc(plot_signal_start);
fprintf('发送和接收信号图形生成完成！耗时: %.2f 秒\n\n', plot_signal_time);
