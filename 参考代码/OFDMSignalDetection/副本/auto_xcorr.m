 ...
 if K==1
     figure
     stem ((1-P)*t :t:( P)*t ,R_tao);   %P*t是将采样点转换为时间表示，有效数据长度变为时间表示
     xlabel('延迟/s');
     ylabel('幅度');
     title('可变长度时域自相关')
     figure
     stem ((1-P) :( P) ,R_tao);   %P*t是将采样点转换为时间表示，有效数据长度变为时间表示
     xlabel('延迟/s');
     ylabel('幅度');
     title('固定长度时域自相关')
 end
 ...
 %取有效数据长度
 %********************************************
 abc=sort( R_tao(1,1:end/2));    %从小到大排序
 Z=length(abc);
 [as1 ,Tu1] =find( R_tao(1,1:end/2)==abc(1,Z-1));%取幅度高度为第二高的峰值对应位置
 ...
 sigma0 = 6;   %观察数据的长度倍数
 L_fft=sigma0*P;     %观察数据的长度，即fft的长度
 RR = zeros(xcorr_len, L_fft);
 data111=[zeros(1,P),data,zeros(1,P)];   %将观察数据长度延长，添加前后零，以便计算固定延时自相关
 for i = 1 : N-4 
    for k=1:L_fft
 ...
 if K==1
     ...
     LL=t:t:20*sigma0*t;
     stem(LL,RRR_fft);
     title('可变固定长度时域自相关的IFFT')
     figure
     stem(RRR_fft);
     title('固定固定长度时域自相关的IFFT')
 end

a=sort(RRR_fft);    %从小到大排序
Z=length(a);
[a1 ,T1] =find(RRR_fft==a(1,Z));%取幅度高度为第一高的峰值对应位置
[a2 ,T2] =find(RRR_fft==a(1,Z-1));%取幅度高度为第二高的峰值对应位置
[a3 ,T3] =find(RRR_fft==a(1,Z-2));%取幅度高度为第三高的峰值对应位置
Ts1 = abs(T2-T1);
Ts2 = abs(T3-T2);
Ts3 = abs(T3-T1);
Tss = [Ts1 Ts2 Ts3];
TT = sort(Tss);  %从小到大排序，取最小值
