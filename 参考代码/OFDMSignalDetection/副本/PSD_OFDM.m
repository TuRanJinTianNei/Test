...
% 计算单边频谱（0到fs/2）
N_fft = length(sig2_chnl);
Frc = (0:N_fft/2-1) * fs / N_fft;  % 单边频率轴：0到fs/2
OfdmSymComput = fft(sig2_chnl, N_fft);  % FFT变换
OfdmSymPSDy = 20 * log10(abs(OfdmSymComput(1:N_fft/2)));  % 只取正频率部分
OfdmSymPSDy = OfdmSymPSDy - max(OfdmSymPSDy);  % 归一化到最大值
