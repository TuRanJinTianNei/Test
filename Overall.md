...
### test6.m - Jakes信道数学特征详解

**功能**：Jakes信道模型的数学特征与参数详解仿真
...
- 基础散射路径数：`N0 = 100`
- 总路径数：`N = 402`
- 随机数生成器：Mersenne Twister（高质量RNG）

**特点**：
- 实现Jakes信道模型，展示其核心数学特征
- 验证Jakes信道的统计特性：
  - **幅度分布**：瑞利分布（Rayleigh distribution）
  - **相位分布**：均匀分布（Uniform distribution）
  - **功率谱**：U型多普勒功率谱
  - **自相关函数**：`J_0(2πf_d Δt)`（零阶贝塞尔函数）
- 生成12个Figure详细展示：
  - Jakes信道时域特征（Figure 1-4）
  - Jakes信道频域特征（Figure 5-7）
  - Jakes信道统计特性验证（Figure 8-10）
  - Jakes信道参数影响分析（Figure 11-12）

**应用场景**：理解Jakes信道模型的数学原理，验证信道模型的统计特性
...
4. **衰落信道分析**：使用test5分析移动通信场景
5. **信道模型理解**：使用test6深入理解Jakes信道
6. **信号处理基础**：使用test7学习复包络概念
...
- `README_test6.md` - test6详细说明（Jakes信道数学原理）
...
- 需要完整BER-SNR曲线：使用test1/test3/test4/test5
