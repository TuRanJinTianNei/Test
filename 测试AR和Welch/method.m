...
%
% 独立性说明：
% 本文件可以独立运行，不依赖其他函数文件。所有需要的自定义函数都在
% 文件内部定义，包括：
...
% 运行要求：
%   - MATLAB基础环境
%   - 通信工具箱（Communications Toolbox）- 用于qammod和awgn函数
%   - 信号处理工具箱（Signal Processing Toolbox）- 用于pwelch、hanning、freqz函数
%
% 使用方法：
%   直接在MATLAB命令窗口运行: method
%   或: run('method.m')
%**************************************************************************
% clc;
% close all;
...
snr = -4:2:20;   %高斯白噪声的信噪比（-4到20dB，步长2dB）
% 
fc = 10e6;   %信号的载波频率
fs = 40e6;   % 采样频率
% 
...
%**************************************************************************
% 绘制估计带宽前的原始基带信号（时域和频域）
%**************************************************************************
fprintf('正在绘制估计带宽前的原始信号时域和频域图...\n');
% 计算信号长度信息
sig_length = length(sig_ofdm);  % 采样点数
sig_duration = sig_length / fs;  % 信号时长（秒）
sig_duration_us = sig_duration * 1e6;  % 转换为微秒
sig_duration_ms = sig_duration * 1e3;  % 转换为毫秒
...
% 绘制原始基带信号的时域和频域图
figure('Name', '估计带宽前的原始基带OFDM信号', 'Position', [100, 100, 1400, 800]);

% 时域信号 - 实部
subplot(2, 2, 1);
time_axis = (0:sig_length-1) / fs * 1e6;  % 转换为微秒
plot(time_axis, real(sig_ofdm), 'b-', 'LineWidth', 0.5);
xlabel('时间 (μs)');
ylabel('幅度');
title(sprintf('原始信号时域 - 实部\n总长度: %d 个采样点, 时长: %.2f μs', ...
    sig_length, sig_duration_us));
grid on;
xlim([time_axis(1), min(time_axis(1) + 50, time_axis(end))]);  % 只显示前50微秒以便观察细节

% 时域信号 - 虚部
subplot(2, 2, 2);
plot(time_axis, imag(sig_ofdm), 'r-', 'LineWidth', 0.5);
xlabel('时间 (μs)');
ylabel('幅度');
title(sprintf('原始信号时域 - 虚部\n总长度: %d 个采样点, 时长: %.2f μs', ...
    sig_length, sig_duration_us));
grid on;
xlim([time_axis(1), min(time_axis(1) + 50, time_axis(end))]);  % 只显示前50微秒以便观察细节
...
freq_axis_neg = (-N_fft/2:N_fft/2-1) * fs / N_fft / 1e6;  % 转换为MHz，包含负频率
fft_sig = fft(sig_ofdm, N_fft);
fft_sig_shifted = fftshift(fft_sig);  % 零频率居中
fft_mag = 20*log10(abs(fft_sig_shifted));
fft_mag = fft_mag - max(fft_mag);  % 归一化到最大值
plot(freq_axis_neg, fft_mag);
xlabel('频率 (MHz)');
ylabel('幅度 (dB)');
title('原始信号频域 - 双边频谱');
grid on;
...
plot([0, 0], [min(fft_mag), max(fft_mag)], 'k--', 'LineWidth', 1, ...
    'DisplayName', '零频率 (基带)');
% 标记理论带宽范围（基带，中心在0）
bandwidth_base = 8e6 / 1e6;  % 8MHz带宽
plot([-bandwidth_base/2, -bandwidth_base/2], [min(fft_mag), max(fft_mag)], 'g--', ...
    'LineWidth', 1.5, 'DisplayName', '理论带宽边界');
plot([bandwidth_base/2, bandwidth_base/2], [min(fft_mag), max(fft_mag)], 'g--', ...
    'LineWidth', 1.5, 'DisplayName', '理论带宽边界');
...
% 频域信号 - 单边频谱（只显示正频率）
subplot(2, 2, 4);
freq_axis_pos = (0:N_fft/2-1) * fs / N_fft / 1e6;  % 转换为MHz，只显示正频率
fft_mag_pos = 20*log10(abs(fft_sig(1:N_fft/2)));
fft_mag_pos = fft_mag_pos - max(fft_mag_pos);  % 归一化到最大值
plot(freq_axis_pos, fft_mag_pos);
xlabel('频率 (MHz)');
ylabel('幅度 (dB)');
title('原始信号频域 - 单边频谱');
grid on;
hold on;
% 标记理论带宽上界（基带，从0开始）
plot([bandwidth_base, bandwidth_base], [min(fft_mag_pos), max(fft_mag_pos)], 'g--', ...
    'LineWidth', 1.5, 'DisplayName', sprintf('理论带宽 %.1f MHz', bandwidth_base));
legend('Location', 'best');
xlim([0, fs/2/1e6]);
fprintf('原始信号图形生成完成！\n\n');

%**************************************************************************
% 绘制输入到带宽估计模型的信号（经过上变频、瑞利衰落和加噪声的OFDM实数信号）
%**************************************************************************
fprintf('正在生成输入信号的时域和频谱图...\n');
snr_plot_signal = 20;  % 用于绘图的SNR值
% 处理信号：与PSD_OFDM_rayleigh函数使用相同的处理流程
% 1. 上变频
sig_processed = (sig_ofdm.*exp(1j*2*pi*fc/fs*(0:length(sig_ofdm)-1)));
% 2. 通过瑞利衰落信道
sig_processed = real(MUL_RAYLEIGH(sig_processed,itau,power,itn,length(itau),length(sig_processed),1/fs,fmax,0));
% 3. 加AWGN噪声
...
% 绘制时域和频谱图
figure('Name', '输入到带宽估计模型的信号（Rayleigh信道）', 'Position', [100, 100, 1400, 600]);
...
xlabel('时间 (μs)');
ylabel('幅度');
title(sprintf('Rayleigh信道信号时域波形 (SNR = %.1f dB)\n总长度: %d 个采样点, 时长: %.2f μs', ...
...
freq_axis = (0:N_fft/2-1) * fs / N_fft / 1e6;  % 转换为MHz
fft_sig = fft(sig_processed, N_fft);
fft_mag = 20*log10(abs(fft_sig(1:N_fft/2)));
fft_mag = fft_mag - max(fft_mag);  % 归一化到最大值
plot(freq_axis, fft_mag);
xlabel('频率 (MHz)');
ylabel('幅度 (dB)');
title(sprintf('Rayleigh信道信号频谱 (SNR = %.1f dB)\n载波频率: %.1f MHz, 理论带宽: 8 MHz', ...
...
%k=1时绘制PSD
%B的标准值8MHz
% 注意：此函数现在调用 estimateBandwidthPSD.m 工具函数进行带宽估计

% 信号处理：上变频、Rayleigh信道、加噪声
