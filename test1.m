...
% 追加图（拆分 + 频域）：第二个OFDM符号加窗前后的时域与频域
figure(12);
subplot(2,1,1);
plot(0:IFFT_bin_length-1+GI+GIP, real(time_wave_matrix_cp(2,:)),'b-');
grid on;
axis([0, IFFT_bin_length-1+GI+GIP, -0.2, 0.2]);
ylabel('Amplitude');
xlabel('Time');
...
subplot(2,1,2);
seq12 = real(time_wave_matrix_cp(2,:));
N12 = length(seq12);
spec12 = abs(fft(seq12));
f12 = (0:(N12-1))/N12; % 归一化频率
plot(f12(1:floor(N12/2)), spec12(1:floor(N12/2)),'b-');
grid on;
xlabel('Normalized Frequency (0.5 = fs/2)');
ylabel('Magnitude');
title('Second OFDM Symbol: Before Windowing (Frequency)');

figure(13);
subplot(2,1,1);
plot(0:IFFT_bin_length-1+GI+GIP, windowed_time_wave_matrix_cp(2,:),'r-');
grid on;
axis([0, IFFT_bin_length-1+GI+GIP, -0.2, 0.2]);
ylabel('Amplitude');
xlabel('Time');
...
subplot(2,1,2);
seq13 = windowed_time_wave_matrix_cp(2,:);
N13 = length(seq13);
spec13 = abs(fft(seq13));
f13 = (0:(N13-1))/N13; % 归一化频率
plot(f13(1:floor(N13/2)), spec13(1:floor(N13/2)),'r-');
grid on;
xlabel('Normalized Frequency (0.5 = fs/2)');
ylabel('Magnitude');
title('Second OFDM Symbol: After Windowing (Frequency)');
